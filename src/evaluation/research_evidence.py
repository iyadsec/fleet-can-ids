...
def save_raw_vs_descriptor_size(
    ...
    *,
    ...
    windows = pd.read_csv(window_features_path, usecols=["window_id"])
    descriptors = pd.read_csv(descriptors_path, usecols=["event_id"])
    total_raw_windows = int(len(windows))
    total_descriptors = int(len(descriptors))
