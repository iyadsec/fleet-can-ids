...
def _write_fpr_validation_report(
    path: Path,
    *,
    metrics_by_vehicle_df: pd.DataFrame,
    comparison_df: pd.DataFrame,
    per_vehicle_f1_optimal: dict[str, dict[str, float]],
    per_vehicle_selected: dict[str, float],
    selected_pool: ThresholdPick,
) -> None:
    """Document threshold policy, superseded high-FPR sources, and corrected metrics."""
    f1_row = comparison_df[comparison_df["threshold_method"] == "F1-optimal"].iloc[0]
    fpr_row = comparison_df[comparison_df["threshold_method"].str.startswith("FPR<=5%")].iloc[0]
    p4_path = Path("experimental-2026-06-23/01_primary_ocslab_balanced/tables/table_P4_vehicle_level_results.csv")
    ...
    lines = [
        ...
        f"| `experimental-2026-06-23/.../table_P4_vehicle_level_results.csv` | Fixed strong thresholds (0.85/0.7) | **~31%** | Fleet pipeline descriptor gate, not FPR-controlled |",
        "| Draft Table I (manuscript) | Likely default / F1-oriented threshold | **~37%** | Inconsistent with deployment policy |",
        "",
        "The archived comparison file `results/vehicle_level_threshold_comparison.csv` shows:",
        f"- **F1-optimal:** test F1={f1_row['test_f1']:.3f}, test FPR={f1_row['test_fpr']:.3f}",
        f"- **FPR≤5% (corrected):** test F1={fpr_row['test_f1']:.3f}, test FPR={fpr_row['test_fpr']:.3f}",
        "",
    ]
    if p4_path.exists():
        p4 = pd.read_csv(p4_path)
        lines.append(f"Nearest archived per-vehicle high-FPR table: `{p4_path}`")
        lines.append("")
        lines.append("```")
        lines.append(p4.to_string(index=False))
        lines.append("```")
        lines.append("")

    lines.extend(
        [
            "## Per-vehicle F1-optimal diagnostic (test, not for publication)",
            "",
        ]
    )
    for vehicle, m in sorted(per_vehicle_f1_optimal.items()):
        lines.append(
            f"- **{vehicle}:** F1={m['f1']:.3f}, FPR={m['fpr']:.3f} "
            f"(threshold would maximise F1 on validation — not used in paper)"
        )
    lines.extend(
        [
            "",
            ...
            "",
            "```",
            metrics_by_vehicle_df.to_string(index=False),
            "```",
            "",
            "## Selected thresholds",
            "",
    ...
    for vehicle, t in per_vehicle_selected.items():
        lines.append(f"- **{vehicle}:** {t:.6f} (validation FPR≤5%)")
    lines.append(f"- **Pooled reference:** {selected_pool.value:.6f} ({selected_pool.method})")
    lines.extend(
        [
            "",
            "## Recommendation",
            ...
            "(or pooled row in `results/vehicle_level_metrics_fpr_controlled.csv`).",
            "Use the FPR≤5% row for the deployment-oriented summary (matches Table X).",
...
def _write_per_vehicle_publication_outputs(
    metrics_by_vehicle_df: pd.DataFrame,
    outputs: EvaluationOutputs,
) -> dict[str, Path]:
    """Write Table I per-vehicle CSV and LaTeX (FPR-controlled protocol)."""
    pub_df = metrics_by_vehicle_df.copy()
    pub_df["vehicle"] = pub_df["vehicle_model"].replace({"pooled": "Pooled"})
    ...
    pub_df = pub_df[["vehicle", "pr_auc", "precision", "recall", "f1", "fpr", "threshold"]]

    vehicle_order = ["Chevrolet", "Hyundai", "Kia", "Pooled"]
    pub_df["vehicle"] = pd.Categorical(pub_df["vehicle"], categories=vehicle_order, ordered=True)
    pub_df = pub_df.sort_values("vehicle").reset_index(drop=True)

    csv_path = outputs.results_dir / "vehicle_level_metrics_per_vehicle_fpr_controlled.csv"
    pub_df.to_csv(csv_path, index=False)

    tex_df = pub_df.rename(
        columns={
            "vehicle": "Vehicle",
            "pr_auc": "PR-AUC",
            "precision": "Precision",
    ...
    tex_path = outputs.tables_dir / "table_vehicle_level_per_vehicle_fpr_controlled.tex"
    tex_path.write_text(
        _df_to_ieee_tex(
            tex_df,
            "Table I: Vehicle-level Isolation Forest performance on the held-out test partition "
            "(benign-only training; validation FPR$\\leq$5\\% threshold selection).",
            "tab:vehicle-level-per-vehicle-fpr",
        ),
        encoding="utf-8",
    )
    return {"per_vehicle_metrics_csv": csv_path, "per_vehicle_table_tex": tex_path}


def _write_per_vehicle_validation_report(
    path: Path,
    *,
    metrics_by_vehicle_df: pd.DataFrame,
    comparison_df: pd.DataFrame,
    ...
    """Answer audit questions for corrected per-vehicle Table I."""
    f1_row = comparison_df[comparison_df["threshold_method"] == "F1-optimal"].iloc[0]
    fpr_row = comparison_df[comparison_df["threshold_method"].str.startswith("FPR<=5%")].iloc[0]
    ...
    pub = metrics_by_vehicle_df.copy()
    pub["vehicle"] = pub["vehicle_model"].replace({"pooled": "Pooled"})
    all_fpr_ok = bool((pub["false_positive_rate"] <= 0.05 + 1e-9).all())
    ...
    lines = [
        ...
        "(`experiments/04_train_vehicle_ids.py` → `src/models/vehicle_ids.py::generate_vehicle_anomaly_predictions`), "
        "which applies **fixed strong/weak score thresholds** (defaults: strong=0.80, Chevrolet strong=0.85 in balanced runs) "
        "rather than validation FPR control.",
        "",
        "Archived high-FPR per-vehicle rows appear in "
        "`experimental-2026-06-23/01_primary_ocslab_balanced/tables/table_P4_vehicle_level_results.csv` "
        "(generated by the balanced publication fleet run, not `run_vehicle_level_evaluation.py`).",
        "",
        "## 2. What threshold policy did old Table I use?",
        "",
        "| Policy | Description | Pooled test FPR (reference) |",
        "|--------|-------------|------------------------------|",
        f"| **Old Table I (manuscript)** | Fixed / F1-oriented strong threshold | **~37%** |",
        f"| F1-optimal on validation | Max validation F1 | **{f1_row['test_fpr']:.1%}** |",
        "| Fixed strong threshold (P4) | Chevrolet 0.85, Hyundai/Kia 0.70 | **~31%** |",
        f"| **Final protocol (this report)** | Validation FPR≤5%, max recall | **{fpr_row['test_fpr']:.1%}** |",
        "",
        "Old manuscript Table I (for reference):",
        "",
        "| Vehicle | PR-AUC | Precision | Recall | F1 | FPR |",
        "|---------|--------|-----------|--------|-----|-----|",
    ]
    for v, pr, prec, rec, f1, fpr in old_table:
        lines.append(f"| {v} | {pr:.3f} | {prec:.3f} | {rec:.3f} | {f1:.3f} | {fpr:.3f} |")
    ...
    lines.extend(
        [
            ...
            "```",
            pub[
                ["vehicle", "pr_auc", "precision", "recall", "f1", "false_positive_rate", "threshold"]
    ...
    if all_fpr_ok:
        lines.append(
            "**PASS:** All per-vehicle and pooled test FPR values are ≤ 5% under the final protocol."
        )
    else:
        lines.append(
            "**CHECK:** One or more test FPR values exceed 5%; review threshold selection."
        )
    for _, row in pub.iterrows():
        lines.append(
            f"- **{row['vehicle']}:** test FPR={row['false_positive_rate']:.4f}, "
            f"threshold={row['threshold']:.6f}"
        )
    lines.extend(
        [
            "",
            "## Selected validation thresholds (FPR≤5%, max recall)",
            "",
        ]
    )
    for vehicle, t in sorted(per_vehicle_selected.items()):
        lines.append(f"- **{vehicle}:** {t:.6f}")
    lines.extend(
        [
            "",
            "## Recommendation",
            "",
            "Replace Table I with `tables/table_vehicle_level_per_vehicle_fpr_controlled.tex` "
            "and `results/vehicle_level_metrics_per_vehicle_fpr_controlled.csv`.",
            "",
        ]
    )
    path.parent.mkdir(parents=True, exist_ok=True)
...
def run_vehicle_level_evaluation(
    ...
    *,
    ...
    per_vehicle_rows: list[dict[str, Any]] = []
        ...
        per_vehicle_rows.append(
            {
                ...
                "roc_auc": round(vm["roc_auc"], 4),
                "pr_auc": round(vm["pr_auc"], 4),
                "precision": round(vm["precision"], 4),
                "recall": round(vm["recall"], 4),
                "f1": round(vm["f1"], 4),
                "false_positive_rate": round(vm["fpr"], 4),
                "threshold": round(per_vehicle_selected[vehicle], 6),
                "threshold_method": "FPR<=5%",
                "detection_latency_ms": round(lat, 2) if not np.isnan(lat) else "",
                "test_windows": len(part),
            }
        )
        # Diagnostic: F1-optimal threshold on this vehicle's validation split
        vval = val_all[val_all["vehicle_model"] == vehicle]
        if not vval.empty:
            yv = vval["label"].to_numpy(dtype=np.int64)
            sv = vval["anomaly_score"].to_numpy(dtype=np.float64)
            t_f1 = _select_threshold_max_f1(yv, sv)
            per_vehicle_f1_optimal[vehicle] = _metrics_at_threshold(
                part["label"].to_numpy(dtype=np.int64),
                part["anomaly_score"].to_numpy(dtype=np.float64),
                t_f1,
            )
    ...
    per_vehicle_rows.append(
        {
            ...
            "threshold": round(float(np.median(list(per_vehicle_selected.values()))), 6),
            "threshold_method": selected_pool.method,
            "detection_latency_ms": round(pooled_latency, 2),
            "test_windows": len(test_eval),
        }
    )
    metrics_by_vehicle_df = pd.DataFrame(per_vehicle_rows)
    ...
    corrected_metrics_csv = outputs.results_dir / "vehicle_level_metrics_fpr_controlled.csv"
    metrics_by_vehicle_df.to_csv(corrected_metrics_csv, index=False)

    scored_splits_csv = outputs.results_dir / "vehicle_level_scored_splits.csv"
    base_cols = [
        c
        for c in [
            "window_id",
    ...
    val_export = val_all[base_cols].copy()
    val_export["split"] = "validation"
    test_export = test_eval[
        base_cols + [c for c in ("predicted_label", "threshold") if c in test_eval.columns]
    ].copy()
    test_export["split"] = "test"
    pd.concat([val_export, test_export], ignore_index=True).to_csv(scored_splits_csv, index=False)

    threshold_rows = [
        {
            "vehicle_model": vehicle,
            "threshold_method": "FPR<=5%",
            "threshold_value": round(t, 6),
    ...
    threshold_rows.append(
        {
            "vehicle_model": "pooled",
            "threshold_method": selected_method_label,
            "threshold_value": round(selected_pool.value, 6),
            "validation_fpr_cap": 0.05,
        }
    )
    thresholds_df = pd.DataFrame(threshold_rows)
    thresholds_csv = outputs.results_dir / "vehicle_level_thresholds_fpr_controlled.csv"
    thresholds_df.to_csv(thresholds_csv, index=False)

    corrected_tex = outputs.tables_dir / "table_vehicle_level_ids_fpr_controlled.tex"
    tex_df = metrics_by_vehicle_df[
        ["vehicle_model", "pr_auc", "precision", "recall", "f1", "false_positive_rate"]
    ].copy()
    tex_df.columns = [
        "Vehicle",
        "PR-AUC",
    ...
    _write_fpr_validation_report(
        outputs.results_dir / "vehicle_level_validation_report.md",
        metrics_by_vehicle_df=metrics_by_vehicle_df,
        comparison_df=comparison_df,
        per_vehicle_f1_optimal=per_vehicle_f1_optimal,
        per_vehicle_selected=per_vehicle_selected,
        selected_pool=selected_pool,
    )

    per_vehicle_pub = _write_per_vehicle_publication_outputs(metrics_by_vehicle_df, outputs)
    per_vehicle_report = outputs.results_dir / "per_vehicle_validation_report.md"
    _write_per_vehicle_validation_report(
        per_vehicle_report,
        metrics_by_vehicle_df=metrics_by_vehicle_df,
        comparison_df=comparison_df,
        per_vehicle_selected=per_vehicle_selected,
    ...
    return {
        ...
        "vehicle_level_metrics_fpr_controlled": corrected_metrics_csv,
        "vehicle_level_threshold_comparison": comparison_csv,
        "vehicle_level_thresholds": threshold_path,
        "vehicle_level_thresholds_fpr_controlled": thresholds_csv,
        "vehicle_level_scored_splits": scored_splits_csv,
        "vehicle_level_validation_report": outputs.results_dir / "vehicle_level_validation_report.md",
        ...
        "table_vehicle_level_ids_fpr_controlled_tex": corrected_tex,
        "vehicle_level_metrics_per_vehicle_fpr_controlled": per_vehicle_pub["per_vehicle_metrics_csv"],
        "table_vehicle_level_per_vehicle_fpr_controlled_tex": per_vehicle_pub["per_vehicle_table_tex"],
        "per_vehicle_validation_report": per_vehicle_report,
